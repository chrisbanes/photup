...
public class MediaStoreAsyncTask extends AsyncTask<Void, Void, List<PhotoSelection>> {
	...
	protected List<PhotoSelection> doInBackground(Void... params) {
		...
		if (null != context) {
			...
			Cursor cursor = MediaStoreCursorHelper.openCursor(context, contentUri);
			if (cursor.getCount() == 0) {
				cursor.close();

				contentUri = Images.Media.INTERNAL_CONTENT_URI;
				cursor = MediaStoreCursorHelper.openCursor(context, contentUri);
			}

			List<PhotoSelection> selection = MediaStoreCursorHelper.cursorToSelectionList(contentUri, cursor);
			cursor.close();
